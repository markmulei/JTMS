...
Ext.define('ExtMVC.view.app.PortalDropZone', {
    ...
    notifyOver: function (dd, e, data) {
        ...
        for (len; pos < len; pos++) {
            ...
            if (h === 0) {
                overSelf = true;
            } else if ((overPortlet.el.getY() + (h / 3)) > xy[1]) {
                match = true;
                break;
            }
        }
        ...
        if (portal.fireEvent('validatedrop', overEvent) !== false && portal.fireEvent('beforedragover', overEvent) !== false) {
            ...
            if (overPortlet) {
                Ext.dd.PanelProxy(overPortlet.el.dom.parentNode, match ? overPortlet.el.dom : null);
            } else {
                Ext.dd.PanelProxy(overColumn.el.dom, null);
